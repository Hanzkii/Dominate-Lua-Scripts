...
function OnPaint()
	...
	if GetIsAlive(LocalPlayer) then
		...
		if FD:GetBool() == true then
			DrawText(5, math.floor(height / 1.5), 0, Color, 0, 255, "Tahoma", 24, "FD", false, true)
		else
		...
		if DT:GetBool() == true  and TicksRecharged > 0 and not FD:GetBool() == true then
			DrawText(5,  math.floor(height / 1.55), 0, Color, 0, 255, "Tahoma", 24, "DT", false, true)
			DrawFilledCircle(45,   math.floor(height / 1.55), math.floor(CircleRad), 0, Color, 0, 255)
		else
			DrawText(5,  math.floor(height / 1.55), 0, 0, 0, 255, "Tahoma", 24, "DT", false, true)
		end

		if OverrideDMG:GetBool() == true then
			DrawText(5, math.floor(height / 1.60), 0, Color, 0, 255, "Tahoma", 24, "Damage", false, true)
		else
